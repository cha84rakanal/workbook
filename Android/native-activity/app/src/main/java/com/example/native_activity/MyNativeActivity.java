 ...
 */
public class MyNativeActivity extends Activity implements SurfaceHolder.Callback2,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mNativeHandle = loadNativeCode(path, funcname, Looper.myQueue(),
                ...
                classLoader, callGetLdLibraryPathOnBaseDexClassLoader(classLoader));
    ...
    protected void onDestroy() {
        ...
        if (mCurInputQueue != null) {
            onInputQueueDestroyedNative(mNativeHandle, callGetNativePtrOnInputQueue(mCurInputQueue));
            mCurInputQueue = null;
        }
    ...
    public void onInputQueueCreated(InputQueue queue) {
        if (!mDestroyed) {
            ...
            onInputQueueCreatedNative(mNativeHandle, callGetNativePtrOnInputQueue(queue));
        }
    }

    public void onInputQueueDestroyed(InputQueue queue) {
        if (!mDestroyed) {
            onInputQueueDestroyedNative(mNativeHandle, callGetNativePtrOnInputQueue(queue));
            mCurInputQueue = null;
        }
    }

    private static String callGetLdLibraryPathOnBaseDexClassLoader(BaseDexClassLoader classLoader) {

        try{
            Class clazz = Class.forName("dalvik.system.BaseDexClassLoader");
            Method method = clazz.getMethod("getLdLibraryPath");
            Object returnValue = method.invoke(classLoader);
